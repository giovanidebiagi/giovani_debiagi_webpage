...
void main() {
  late MockMechatronicsProjectsDataLocalDatasource _mockMechatronicsProjectsDataLocalDatasource;
  late IMechatronicsProjectsRepository _mechatronicsProjectsRepository;

  setUp(() {
    _mockMechatronicsProjectsDataLocalDatasource = MockMechatronicsProjectsDataLocalDatasource();
    _mechatronicsProjectsRepository =
        MechatronicsProjectsRepository(datasource: _mockMechatronicsProjectsDataLocalDatasource);
  });

  final List<String> _mechatronicsProjectsImagesPaths =
      List<String>.generate(8, (index) => '/path/to/mechatronics/project/image/$index');
  final List<MechatronicsProject> _flutterProjectsList = List<MechatronicsProject>.generate(
    8,
  ...
  test('should return Mechatronics Projects list when fetching is successful', () async {
    // arrange
    when(() => _mockMechatronicsProjectsDataLocalDatasource.getMechatronicsProjects())
        .thenAnswer((invocation) async => _flutterProjectsList);

    // act
    final _result = await _mechatronicsProjectsRepository.getMechatronicsProjects();

    // assert
    verify(() => _mockMechatronicsProjectsDataLocalDatasource.getMechatronicsProjects());
    expect(_result, Right(_flutterProjectsList));
  });

  test('should return [CacheFailure] when fetching is not successful', () async {
    // arrange
    when(() => _mockMechatronicsProjectsDataLocalDatasource.getMechatronicsProjects())
