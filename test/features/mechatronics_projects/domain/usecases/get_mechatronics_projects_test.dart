...
class MockMechatronicsProjectsRepository extends Mock
...
void main() {
  ...
  test(
      'should return Mechatronics Projects list when getting mechatronics projects is successful',
      () async {
    // arrange
    final List<String> _mechatronicsProjectImagesPaths = List<String>.generate(
        8, (index) => '/path/to/mechatronics/project/image/$index');
    final List<MechatronicsProject> _mechatronicsProjectsList =
  ...
  test(
      'should return [CacheFailure] when getting mechatronics projects is not successful',
      () async {
    // arrange
    when(() => _mockMechatronicsProjectsRepository.getMechatronicsProjects())
